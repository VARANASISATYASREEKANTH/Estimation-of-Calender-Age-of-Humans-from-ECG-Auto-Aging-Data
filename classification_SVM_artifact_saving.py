# -*- coding: utf-8 -*-
"""
Created on Mon Dec  8 13:32:45 2025
...
import joblib # For saving the scikit-learn model and artifacts
import time # To measure training time
from tqdm import tqdm # Import tqdm for progress visualization

# --- Configuration (UPDATED FOR ARTIFACT PATHS) ---
...
# New Artifact Paths
SCALER_PATH = os.path.join(OUTPUT_DIR, 'feature_scaler_SVM.joblib')
ENCODER_PATH = os.path.join(OUTPUT_DIR, 'label_encoder_SVM.joblib')
...
def test_and_save_results(model, X_test_scaled, y_test_true, test_file_ids, label_encoder):
    ...
    X_test_list = np.array_split(X_test_scaled, X_test_scaled.shape[0]) 
...
# ----------------------------------------------------------------------
## 🚀 Main Execution (MODIFIED TO SAVE SCALER AND ENCODER)
# ----------------------------------------------------------------------

def main():
    """
    Main function to execute the classification pipeline using SVM with tqdm.
    Includes saving of StandardScaler and LabelEncoder.
    """
    ...
    # 2. Label Encoding (Fit and SAVE)
    label_encoder = LabelEncoder()
    y_train_encoded_indices = label_encoder.fit_transform(y_train)
    num_classes = len(label_encoder.classes_)
    
    # --- ARTIFACT SAVE 1: Label Encoder ---
    print(f"\nSaving fitted LabelEncoder to: {ENCODER_PATH}")
    joblib.dump(label_encoder, ENCODER_PATH)
    # ------------------------------------

    # 3. Feature Scaling (Fit and SAVE)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    
    # --- ARTIFACT SAVE 2: Standard Scaler ---
    print(f"Saving fitted StandardScaler to: {SCALER_PATH}")
    ...
    # 7. Apply the SAME Scaling to Test Data
